...
class EmbeddingNetwork(nn.Module):
    ...
    def __init__(self, pretrained: bool = True, embedding_dim: int = 768):
        ...
        self.embedding_dim = embedding_dim      #this is a bit redundant but keeping it for consistency with other main.py files so that there is not a unique one just for fully tuned ViT model
        self.embedding_dim = 768                #hard coding this to 768 as we are using ViT_B_16 model only (doing this only for fully tuned model, the others keep the 512 value)

        '''
        layer here unlike other models. But we kept it for consistency across all main.py 
        files and for sanity reasons 
        '''
